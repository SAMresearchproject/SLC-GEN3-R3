...
PREMISES = ROOT / "CR022_declared_premises.json"
...
def build_manifest(premises):
    ...
    rows.append({
        "source_key": "runner",
        "path": str(ROOT / "CR022_runner.py"),
        "role": "courtroom_calculation_script",
        "used_as_computed_input": True,
        "exists": True,
        "sha256": sha256(ROOT / "CR022_runner.py"),
    })
    return rows, loaded
...
def main():
    ...
    write_csv(ROOT / "CR022_input_manifest.csv", manifest)
    write_csv(ROOT / "CR022_evidence_rows.csv", evidence)
