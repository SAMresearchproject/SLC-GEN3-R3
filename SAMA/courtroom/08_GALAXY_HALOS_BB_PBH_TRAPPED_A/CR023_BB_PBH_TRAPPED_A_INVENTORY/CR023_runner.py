...
PREMISES = ROOT / "CR023_declared_premises.json"
...
def build_manifest(premises):
    ...
    rows.append({
        "source_key": "runner",
        "path": str(ROOT / "CR023_runner.py"),
        "role": "courtroom_calculation_script",
        "used_as_computed_input": True,
        "exists": True,
        "sha256": sha256(ROOT / "CR023_runner.py"),
    })
    return rows, loaded
...
def main():
    ...
    write_csv(ROOT / "CR023_input_manifest.csv", manifest)
    write_csv(ROOT / "CR023_evidence_rows.csv", evidence)
