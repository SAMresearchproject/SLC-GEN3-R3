...
PREMISES = ROOT / "CR027_declared_premises.json"
...
def build_manifest(premises):
    ...
    rows.append({
        "source_key": "runner",
        "path": str(ROOT / "CR027_runner.py"),
        "role": "courtroom_calculation_script",
        "used_as_computed_input": True,
        "exists": True,
        "sha256": sha256(ROOT / "CR027_runner.py"),
    })
    return rows, loaded
...
def main():
    ...
    write_csv(ROOT / "CR027_input_manifest.csv", manifest)
    write_csv(ROOT / "CR027_evidence_rows.csv", evidence)
    ...
    (ROOT / "CR027_summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    ...
    (ROOT / "CR027_result.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    write_hashes()
