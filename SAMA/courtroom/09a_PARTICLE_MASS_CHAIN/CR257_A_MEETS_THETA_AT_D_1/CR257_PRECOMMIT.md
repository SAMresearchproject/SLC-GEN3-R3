...
PASS:
  ...
  (5) W4 power-variants on the squared factor 0/8 each
...
The A-Θ meeting at d=1 is a **structural feature** of the antimatter
sector, not an arithmetic accident:
- The matter surface correction `(1 + p/R²)` appears once in the
  matter law and once in the A-operator route correction at d=1
  (where `R^(d+1) = R²` aligns the scales).
- The resulting d=1 compact forms (`(1 + p/R²)²` for neg,
  `(1 − p²/R⁴)` for pos) are substrate-natural products of two
  factor expressions that only coincide at d=1.
- At d≠1, the two scales don't coincide, the compact forms don't
  emerge, and applying the d=1 form elsewhere fails empirically.

This is the structural reading: A-operator and Θ-bridge are non-row
substrate entities that meet at d=1, with measurable consequences in
the antimatter qA values.
...
correction APPEARS A SECOND TIME from the A-operator at d=1.
```
