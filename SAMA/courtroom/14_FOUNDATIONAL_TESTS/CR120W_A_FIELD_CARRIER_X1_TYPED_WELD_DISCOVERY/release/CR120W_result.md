# CR120W A-field-carrier / X1 typed-weld discovery result

## Primary verdict

`PASS_A_FIELD_CARRIER_UNIQUE_TYPED_X1_WELD_CANDIDATE__DIRECT_A_TO_AXIS_EDGE_OPEN`

## Typed candidate result

All **26** partition-one QP093A occurrences received the same
four frozen gates. The census narrowed as follows:

```text
partition-one occurrences          = 26
legal nonlocal infrastructure      = 3
neutral closed carrier-axis shell  = 2
non-additive address role          = 1
shared environmental axis survivor = 1
```

The unique survivor is **QP093A-0305 —
A_FIELD_CARRIER**.
...
```text
twelve additive modes             = 81
A-field historical proxy restored = 82  [wrong control]
mirror/checksum                    = 81
closed ledger                      = 162
wrong restored closure             = 163
```
...
- Frozen source hashes: **25/25** matched.
- Wrong controls: **12/12** passed.
