"""
CR001b@19_PEEBLES_NONEQUILIBRIUM_RECOMBINATION_APPEAL runner.

Implements the test specified in CR001b_PRECOMMIT.md (SHA-256
38259de487a8b3a31df787c7c5d5bed5e442c134472b74e3f5ad0a98d7ff991b).

CHANGE from CR001@19: Saha equilibrium ionization replaced with Peebles
(1968) effective-three-level atom (H-only). Substrate inputs, measurement
inputs, gate thresholds, evidence set, and wrong-control structure are
unchanged from CR001@19.

Outputs:
  CR001b_summary.json
  CR001b_evidence_rows.csv
"""
...
H_NU_2_J = 3.4 * EV_J         # 3.4 eV n=2 binding energy
LAMBDA_ALPHA = 1.215682e-7    # m, Lyman-alpha wavelength
LAMBDA_2GAMMA = 8.227         # s^-1, two-photon decay rate
...
# ============================================================================
# Peebles ODE for x_e(z)
# ============================================================================

Z_PEEBLES_START = 1800.0    # Saha already well below 1 here; well-conditioned IC
Z_PEEBLES_END   = 10.0
ION_E_J = 13.6 * EV_J       # Full hydrogen ionization energy (for Saha-13.6 IC)
...
def beta_2_n2(T_K):
    """Photoionization rate from n=2 (with 3.4 eV binding). Returns s^-1
    per H atom in n=2. Used inside the C-factor denominator."""
    T_J = K_B * T_K
...
def beta_eff_13p6(T_K):
    """Effective ionization rate that drives Peebles equilibrium toward Saha-13.6:
    beta_eff = beta_2 * exp(-h*nu_alpha/kT) = alpha_B * de_broglie * exp(-13.6 eV/kT).
    Returns s^-1 per ground-state H atom."""
    T_J = K_B * T_K
    a_B = alpha_B_recomb(T_K)
    de_broglie = (M_E * T_J / (2.0 * PI * HBAR**2)) ** 1.5
...
def peebles_rhs(z, x_e_arr, Omega_m, Omega_r, Omega_L, h_si, rho_c_0, Omega_b):
    ...
    a_B = alpha_B_recomb(T_K)
    b_2 = beta_2_n2(T_K)            # for C-factor denominator
    b_eff = beta_eff_13p6(T_K)      # for the net rate (drives Saha-13.6 eq)
...
def saha_xe(z, Omega_b, rho_c_0):
    """Standard Saha (13.6 eV) equilibrium ionization fraction. Used as IC
    at Z_PEEBLES_START where Saha and Peebles equilibria agree."""
    a = 1.0 / (1.0 + z)
    T_K = T_0 / a
    T_J = K_B * T_K
    n_H = Omega_b * rho_c_0 / (M_P * a**3)
...
def solve_peebles(Omega_m, Omega_b, Omega_r, Omega_L, h_si, rho_c_0):
    """Solve Peebles ODE from Z_PEEBLES_START down to Z_PEEBLES_END."""
...
# ============================================================================
# THERMAL LADDER with Peebles x_e(a)
# ============================================================================
...
def thermal_ladder(Omega_m, Omega_b, H0_kms_Mpc):
    ...
    # Build x_e on a-grid:
    #   z > Z_PEEBLES_START (very early, a small): x_e = 1.0
    #   Z_PEEBLES_END <= z <= Z_PEEBLES_START: interpolate from Peebles solve
    #   z < Z_PEEBLES_END (a > 1/(1+Z_END)): x_e frozen at last Peebles value
    z_grid = 1.0 / a_grid - 1.0
    a_at_zstart = 1.0 / (1.0 + Z_PEEBLES_START)
    a_at_zend   = 1.0 / (1.0 + Z_PEEBLES_END)

    # Interpolate in z (z_peebles is monotonically decreasing for our integration direction)
...
# ============================================================================
# BBN (schematic Wagoner, identical to CR001@19)
# ============================================================================
...
def main():
    ...
    print("=" * 78)
    print("CR001b@19 -- Peebles non-equilibrium recombination appeal of CR001@19")
    print("=" * 78)
    print(f"\nSubstrate (identical to CR001@19):")
    print(f"  Omega_m  = 1/pi             = {OMEGA_M_SAM:.6f}")
    print(f"  Omega_b  = 2*A_0*(1-chi)    = {OMEGA_B_SAM:.6f}")
    print(f"  H_0      = {H0_KMS_MPC}")
    print(f"  T_0      = {T_0}")
    print(f"  N_eff    = {N_EFF}")
    print(f"\nRecombination model: Peebles (1968) effective-three-level atom (H-only)")
    print(f"                     Pequignot-Petitjean alpha_B; Lambda_2gamma = 8.227 s^-1")
    ...
    print(f"  T_f MeV       = {bbn['T_f_MeV']:.4f}")
    ...
    if gates_pass:
        verdict = "PASS"
        verdict_reason = "P1, P2, P3 all hold under canonical substrate inputs with Peebles recombination"
    else:
        verdict = "FAIL"
    ...
    print(f"\n  CR001b@19 verdict: {verdict}")
    print(f"  reason: {verdict_reason}")

    summary = dict(
        precommit_sha256="38259de487a8b3a31df787c7c5d5bed5e442c134472b74e3f5ad0a98d7ff991b",
        appeal_of="CR001@19",
        appeal_basis="Saha first-approximation replaced with Peebles non-equilibrium effective-three-level atom",
        substrate=dict(
    ...
    (out_dir / "CR001b_summary.json").write_text(json.dumps(summary, indent=2, default=str))
    ...
    with (out_dir / "CR001b_evidence_rows.csv").open("w", newline="") as f:
