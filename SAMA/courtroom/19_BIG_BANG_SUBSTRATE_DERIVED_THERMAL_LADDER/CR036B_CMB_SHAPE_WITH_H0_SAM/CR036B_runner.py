"""
CR036B runner: CR035A2 CMB shape pipeline rerun with H_0 = H_0_SAM.

Implements the test specified in CR036B_PRECOMMIT.md
(SHA-256 ab2fea4b0822412cc5ca9978bab89822ea6fccffa76f94527bff05aaa660305e).

Same SAM density spine, same fixed Planck perturbations, same CAMB engine,
same Planck PR3 data files, same peak finder (windows + prominence +
smoothed-curve heights), same verdict tree (PASS / BOUNDARY (i)(ii)(iii) /
FAIL with Run B polarization sanity guard), same chi^2 / dof / tolerances /
Run B bounds as CR035A2.

ONE input changes:
  CR035A2:  H_0 = 68.76          (externally set)
  CR036B:   H_0 = 67.2503751950  (sealed from CR036; numeric literal
                                  cited from CR036 precommit
                                  345a1a8dc180eb6b28141114a82315e3b8d92889537c1219eec4312d59ca33f2
                                  by value; CR036 file NOT opened)
...
def _guarded_open(file, *args, **kwargs):
    ...
    for pat in FORBIDDEN_PATTERNS:
        if pat.search(base):
            raise RuntimeError(
                f"CR036B forbidden-file guard tripped: attempted to open {p!r}"
            )
...
# The CR036B runner does NOT open any CR036 file at runtime.
...
def main() -> int:
    print(f"CR036B runner  -  CR035A2 CMB shape pipeline rerun with H_0 = H_0_SAM")
    ...
    if p1["p1_pass"] and p2_token == "PASS":
        if pol_debt:
            ...
            verdict_token = (
                "CR036B_BOUNDARY_POLARIZATION_DEBT_TT_PASSES_TE_OR_EE_DEGRADED"
            )
        else:
            verdict = "PASS"
            verdict_case = "PASS"
            verdict_token = (
                "CR036B_PASS_SAM_DENSITY_SPINE_REPRODUCES_PLANCK_TT_FULL_PER_MULTIPOLE_SHAPE"
        ...
        verdict_token = (
            "CR036B_BOUNDARY_TT_FULL_SHAPE_BETWEEN_2_AND_3_CHI2_PER_DOF"
        )
    else:
        ...
        if rescue:
            ...
            verdict_token = (
                "CR036B_BOUNDARY_RUNA_NOT_PASS_BUT_RUNB_RESCUES_WITHIN_5SIGMA_PLANCK_POSTERIOR"
            ...
            verdict_case = "FAIL"
            verdict_token = (
                "CR036B_FAIL_SAM_DENSITY_SPINE_FULL_PER_MULTIPOLE_SHAPE_OUTSIDE_TOLERANCES"
            )

    print(f"\n=== CR036B VERDICT: {verdict}  ({verdict_case}) ===")
    ...
    out_summary = CR_DIR / "CR036B_summary.json"
    out_summary.write_text(json.dumps(summary, indent=2, default=_jsonify), encoding="utf-8")
    print(f"\nWrote: {out_summary.name}")

    write_residual_csv(CR_DIR / "CR036B_TT_residuals.csv",
                       A["ell_v_TT"], A["obs_TT"], A["sig_v_TT"],
                       A["th_v_TT"], A["res_TT"])
    write_residual_csv(CR_DIR / "CR036B_TE_residuals.csv",
                       A["ell_v_TE"], A["obs_TE"], A["sig_v_TE"],
                       A["th_v_TE"], A["res_TE"])
    write_residual_csv(CR_DIR / "CR036B_EE_residuals.csv",
                       A["ell_v_EE"], A["obs_EE"], A["sig_v_EE"],
                       A["th_v_EE"], A["res_EE"])
    write_peaks_csv(CR_DIR / "CR036B_peaks.csv",
                    A["peaks_theory"], A["peaks_planck"])
    write_theory_csv(CR_DIR / "CR036B_runA_theory_spectra.csv",
                     A["ell_th"], A["TT"], A["TE"], A["EE"])
    (CR_DIR / "CR036B_runB_diagnostic.json").write_text(
    ...
    pass_summary_line = {
        "PASS":     "CR036B PASS confirms that the SAM density spine + "
                    "H_0_SAM = 67.2503751950 (sealed from CR036) + Planck 2018 "
                    "posterior-centroid perturbation parameters, run through "
                    "CAMB 1.6.6, reproduces the Planck 2018 PR3 full "
                    "per-multipole bandpower table at the CR035A2 P1 peak "
                    "structure and P2 full-shape tolerances.",
        "BOUNDARY": f"CR036B BOUNDARY ({verdict_case}): SAM density spine + "
                    f"H_0_SAM + Planck-centroid perturbations are consistent "
                    f"with the Planck full per-multipole bandpower table within "
                    f"the CR036B boundary conditions; see the verdict-case "
                    f"detail for the specific gate.",
        "FAIL":     "CR036B FAIL rejects the SAM density spine + H_0_SAM "
                    "cascade at the Planck full per-multipole shape level "
                    "under the CR036B verdict tree.",
    }[verdict]

    result_md = f"""# CR036B_CMB_SHAPE_WITH_H0_SAM
...
"""
    (CR_DIR / "CR036B_result.md").write_text(result_md, encoding="utf-8")
