...
def framing(markers, data_intervals=6):
    if type(data_intervals) is not int or data_intervals < 1:
        raise ValueError('Positive data interval count required')
    marker_count = 4 + data_intervals
    frames = []
    for start in range(max(0,len(markers)-marker_count+1)):
        block = markers[start:start+marker_count]
        ...
        if any(n<1 or abs(v-n)>F(3,25) for v,n in zip(ratios,bins)):
            continue
        frames.append({'marker_indices':list(range(start,start+marker_count)), 'unit_samples':unit,
            'interval_ratio_bins':bins,'ratios':[str(v) for v in ratios],
            'sample_spans':[[block[i],block[i+1]] for i in range(3,marker_count-1)]})
    return frames
...
def symbolize(residual, amplitude, data_intervals=6):
    ...
    frames = framing(markers, data_intervals)
    ...
    return {'data_intervals':data_intervals,'threshold':str(threshold),'markers':markers,'marker_evidence':kept,
...
def tests(native, symbols, permutations=99, seed=20260917):
    data_intervals = symbols.get('data_intervals',6)
    ...
    for index in range(permutations):
        ...
        frames = framing(surrogate,data_intervals) if gaps else []
    ...
    return {'observed_repetition':observed,'permutation_p':str(native[p_id]),
        ...
        'synchronization':{'format':f'four unit-spaced markers then {data_intervals} interval symbols','searched_starts':max(0,len(markers)-(3+data_intervals))},
